...
//            }
    ...
    private function addShuiYin($file){

        $fileName = $file["rootPath"].$file["savepath"].$file["savename"];
        if(substr_count($_SERVER["HTTP_REFERER"],"/admin/adv") == 1){

            list($width, $height, $type, $attr) = getimagesize($fileName);

            //$targetImg = imagecreatetruecolor($width,$height+45);

            //$bg = imagecolorallocatealpha($targetImg , 0 , 0 , 0 , 127);

            //imagealphablending($targetImg,false);

            //imagefill($targetImg,0 , 0 , $bg);
            ...
            switch($file["ext"]){
                ...
                case "jpg":
                    $targetImg = imagecreatefromjpeg($fileName);
                    break;
                case "png":
                    $targetImg = imagecreatefrompng($fileName);
                    break;
                default:
                    return true;//非png和jpg图片不加广告标签
                    break;
            }

            //@imagecopy($targetImg,$srcImg,0,0,0,0,$width,$height);

            $shuiYinImg = imagecreatefrompng("./Public/images/adv_shuiyin.png");

            @imagecopy($targetImg,$shuiYinImg,$width-73-5,5,0,0,73,35);

            imagesavealpha($targetImg , true);
            header("Content-type: image/png;charset=utf-8");
            imagepng($targetImg,$fileName);
        }
    }
