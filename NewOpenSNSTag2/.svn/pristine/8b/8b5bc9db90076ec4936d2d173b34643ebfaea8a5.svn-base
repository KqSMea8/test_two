...
namespace Common\Lib;
     ...
     */
//设置默认时区
use Lib\LogToOss\lib\requestcore\ResponseCore;
use Lib\LogToOss\util\MimeTypes;
use Lib\LogToOss\util\OSS_Exception;
use Lib\LogToOss\util\OSSUtil;
use Lib\LogToOss\lib\requestcore\RequestCore;
use Think\Exception;
...
require_once OSS_API_PATH.DIRECTORY_SEPARATOR.'LogToOss'.DIRECTORY_SEPARATOR.'conf.inc.php';
require_once OSS_API_PATH.DIRECTORY_SEPARATOR.'LogToOss'.DIRECTORY_SEPARATOR.'lib'.DIRECTORY_SEPARATOR.'requestcore'.DIRECTORY_SEPARATOR.'Requestcore.class.php';
require_once OSS_API_PATH.DIRECTORY_SEPARATOR.'LogToOss'.DIRECTORY_SEPARATOR.'util'.DIRECTORY_SEPARATOR.'mimetypes.class.php';
require_once OSS_API_PATH.DIRECTORY_SEPARATOR.'LogToOss'.DIRECTORY_SEPARATOR.'util'.DIRECTORY_SEPARATOR.'oss_util.class.php';
//检测语言包
if(file_exists(OSS_API_PATH.DIRECTORY_SEPARATOR.'LogToOss'.DIRECTORY_SEPARATOR.'lang'.DIRECTORY_SEPARATOR.ALI_LANG.'.inc.php')){
    require_once OSS_API_PATH.DIRECTORY_SEPARATOR.'LogToOss'.DIRECTORY_SEPARATOR.'lang'.DIRECTORY_SEPARATOR.ALI_LANG.'.inc.php';
}else{
...
class LogToOss
{
    ...
    public function LogToOss($object,$filePath)
    {

        $bucket = 'jpgk';

        $options = array(
            $this::OSS_HEADERS => array(
        ...
        $response = $this->upload_file_by_file($bucket,$object,$filePath,$options);
        return $response;
    }
    /**
     * 拷贝object
    ...
    public function delete_object($bucket, $object, $options = NULL){
        $this->precheck_common('jpgk', $object, $options);
        $options[self::OSS_BUCKET] = $bucket;
        $options[self::OSS_METHOD] = self::OSS_HTTP_DELETE;
        $options[self::OSS_OBJECT] = $object;
        return $this->auth($options);
    ...
    //OSS 内部常量
    const OSS_BUCKET = 'jpgk';
    const OSS_OBJECT = './log';
