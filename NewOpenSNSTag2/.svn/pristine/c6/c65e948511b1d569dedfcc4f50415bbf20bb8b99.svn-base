...
class SourceController extends AdminController
{
    ...
    public function forum($page = 1, $r = 20)
    {
        ...
        $builder
            ...
            ->buttonNew(U('Source/editForum'))
            ->setStatusUrl(U('Source/setForumStatus'))->buttonEnable()->buttonDisable()->buttonDelete()
            ->buttonSort(U('Source/sortForum'))
            ->keyId()->keyLink('title', L('_TITLE_'), 'Source/post?forum_id=###')
    ...
    public function post($page = 1, $forum_id = null, $r = 20, $title = '', $content = '')
    {
      ...
      for($i=0;$i<count($forum1);$i++)
      {
        ...
        $builder->title(L('_POST_MANAGE_') . $forumTitle)
            ->setStatusUrl(U('Source/setPostStatus'))->buttonEnable()->buttonDisable()->buttonDelete()->buttonModalPopup(U('Source/changePlate', array('forum_id'=>$map['forum_id'])), array(), L('_MIGRATING_NOTE_'),array('data-title'=>L('_MIGRATING_NOTE_TO_ANOTHER_PLATE_'),'target-form'=>'ids'))
            ->selectPlateForm('spf',get,U('Admin/Source/post'))->select($title = '筛选：', $name = 'forum_id','select', '', '', '',$forum1)
            ->keyId()->keyLink('title', L('_TITLE_'), 'Source/reply?post_id=###')
            ...
            ->setSearchPostUrl(U('Admin/Source/post'))->search(L('_TITLE_'), 'title')->search(L('_CONTENT_'), 'content')
            ->data($list)
            ->pagination($totalCount, $r)
        ->display();
    }
    ...
    public function postTrash($page = 1, $r = 20,$forum_id = null)
    {
        ...
        $builder->title(L('_REPLY_VIEW_MORE_'))
            ->setStatusUrl(U('Source/setPostStatus'))->buttonRestore()->buttonClear('ForumPost')
            ->keyId()->keyLink('title', L('_TITLE_'), 'Source/reply?post_id=###')
