...
//        print_r($_FILES);die;
        ...
        //上传lz阿里云
        $log = new LogToOss();
        $object = 'catering'.$info['file']['savepath'] . $info['file']['savename'];
        $filePath = '.'.$info['file']['savepath'] . $info['file']['savename'];
        if(!empty($filePath)){
            $log->LogToOss($object,$filePath);
        }

//        $log = new LogToOss();
//        $object = 'catering/Uploads/Download/2017-03-07/app-release(H).apk';
//        $filePath = './Uploads/Download/2017-03-07/app-release(H).apk';
//        if(!empty($filePath)){
//            $return = $log->LogToOss($object,$filePath);
//        }
//        print_r($return);die;
    ...
    public function uploadPicture()
    {
        ...
        //上传lz阿里云
        $log = new LogToOss();
        $object = 'catering'.$info['download']['path'];
        $filePath = $info['download']['rootPath'] . $info['download']['savepath'] . $info['download']['savename'];
        if(empty($filePath)){
            $filePath = '.'.$info['download']['path'];
        }
        $log->LogToOss($object,$filePath);
     ...
     * @author huajie <banhuajie@163.com>
     */
    public function uploadPictureNew(){
...
//echo 1111;die;
        ...
        //上传lz阿里云
        $log = new LogToOss();
        $object = 'catering'.$info['file']['path'];
        $filePath = $info['file']['rootPath'] . $info['file']['savepath'] . $info['file']['savename'];
        if(!empty($filePath)){
            $log->LogToOss($object,$filePath);
        }
        ...
        if ($info) {
            $return['data'] = $info;
//            $return['data']['file']['name'] = $_FILES['file']['name'];
            $return['data']['file']['name'] = $info['file']['path'];
        } else {
            $return['status'] = 0;
    ...
    public function uploadPictureUM()
    {
        ...
        //用于图片之前上传过 但没上传到阿里云 重新上传使用
        if(!$info['upfile']['savepath']){
            $newPath = explode('/',$info['upfile']['path']);
            $info['upfile']['savepath'] = $newPath[4].'/';
            $info['upfile']['savename'] = $newPath[5];
            $info['upfile']['rootPath'] = $setting['rootPath'];
        }

        //上传lz阿里云
        $log = new LogToOss();
        $object = 'catering'.$info['upfile']['path'];
        $filePath = $info['upfile']['rootPath'] . $info['upfile']['savepath'] . $info['upfile']['savename'];
        if(!empty($filePath)){
            $log->LogToOss($object,$filePath);
        }
        ...
        if ($info) {
            ...
            $rs['state'] = 'SUCCESS';
            $rs['url'] = "http://image.lz517.com/catering".$info['upfile']['path'];
