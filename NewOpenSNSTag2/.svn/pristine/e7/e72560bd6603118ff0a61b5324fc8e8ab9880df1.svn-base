    ...
    public function getAvatar($uid = 0, $size = 256)
    {
        ...
        if ($avatar) {
            ...
            if($avatar['driver'] == 'local'){
				...
				if(substr_count($avatar["path"],"/Uploads/Avatar")>=1){

					return "http://image.lz517.com/catering".$avatar['path'];
				}else{

					$avatar_path = "/Uploads/Avatar".$avatar['path'];
					$path = $this->getImageUrlByPath($avatar_path, $size);
					$path = "http://opensns.lz517.com/".$path;
