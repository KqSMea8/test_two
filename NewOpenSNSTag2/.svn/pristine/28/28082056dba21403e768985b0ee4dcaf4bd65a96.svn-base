...
//test_output_json($picture);
        ...
        $tjNews = [];
        $coverIdArr = [];
        $tuiJianNews = $this->newsModel->where(["category" => $info["category"],"status" => 1,"publish_time" => ["lt",time()]])->order("create_time desc")->limit(0,3)->select();
