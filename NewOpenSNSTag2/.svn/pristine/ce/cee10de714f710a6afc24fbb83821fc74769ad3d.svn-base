...
$(function () {
    ...
    //ajax get请求
    $('.ajax-get-classRoom').click(function () {
        var target;
        var that = this;
        if ($(this).hasClass('confirm')) {
            if (!confirm('删除时该分类下的数据也会删除 是否继续?')) {
                return false;
            }
        }
        if ((target = $(this).attr('href')) || (target = $(this).attr('url'))) {
                ...
                if (data.status == 1) {
                    ...
                    setTimeout(function () {
                        ...
                        if(data.formData != ""){
                            ...
                            $("body").html(data.formData);
                            //newTab.document.write(data.formData);
                            return true;
                        }else if(data.openNewUrl != ""){

                            window.open(data.openNewUrl);
                        }else if (data.url) {
                            location.href = data.url;
                        } else if ($(that).hasClass('no-refresh')) {
                            $('#top-alert').find('button').click();
