...
//dump($aVerify);
        ...
        if (IS_POST) { //注册用户
            ...
            if (mb_strlen($aNickname,'utf-8') < intval(modC('NICKNAME_MIN_LENGTH', '2', 'USERCONFIG'))) {
                $this->error('昵称请不少于' . modC('NICKNAME_MIN_LENGTH', '2', 'USERCONFIG') . '个字符');
            }
            if (mb_strlen($aNickname,'utf-8') > modC('NICKNAME_MAX_LENGTH', '32', 'USERCONFIG')) {
                $this->error('昵称请不多于' . modC('NICKNAME_MAX_LENGTH', '32', 'USERCONFIG') . '个字符');
            }
            if ($aRegType == 'username') {
                if (mb_strlen($aUsername,'utf-8') < modC('USERNAME_MIN_LENGTH', '2', 'USERCONFIG')) {
                    $this->error('用户名请不少于' . modC('USERNAME_MIN_LENGTH', '2', 'USERCONFIG') . '个字符');
                }
                if (mb_strlen($aUsername,'utf-8') > modC('USERNAME_MAX_LENGTH', '32', 'USERCONFIG')) {
    ...
    public function foundPassWord()
    {
        ...
        $aMobile = $mobile = I('post.mobile', '', 'op_t');
     //   $aUsername = $username = I('post.username', '', 'op_t');
...
/*                $user = UCenterMember()->where(array('username' => $username, 'status' => 1))->find();
                $uid = $user['id'];
                if (!$uid) {
                    $this->ajaxReturn(array('status' => 0, 'info' => '用户名错误'));
                }*/
