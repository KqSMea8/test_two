...
//        }
     ...
     * QQ:2322523770
     */
    public function delDeviceForInspectionIng($deviceId,$inspectionChildDeviceId){

        //删除设备数据
        if($deviceId){
            (new WechatDeviceModel())->where(['id'=>$deviceId])->delete();
        }

        if($inspectionChildDeviceId){

            $WechatInspectionStoreChildDeviceModel = (new WechatInspectionStoreChildDeviceModel());
            $info = $WechatInspectionStoreChildDeviceModel->where([
            ...
            if($info){
                //报修
                if($info['repairs_id']){
                    $this->cancelOrder($info['repairs_id']);
                }

                //已修
                if($info['yet_repairs_id']){
                    $this->cancelOrder($info['yet_repairs_id']);
                }

                //删除巡检设备表数据
                $WechatInspectionStoreChildDeviceModel->where(['inspection_child_device_id'=>$inspectionChildDeviceId])->delete();
            }
        }
    ...
    private function cancelOrder($orderId){
        ...
        (new WechatOrderModel())->where(['id'=>$orderId])->save(['order_state'=>PAY_STATUS_PAY_FAIL]);
