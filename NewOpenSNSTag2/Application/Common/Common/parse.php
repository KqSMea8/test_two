 ...
 * autor:HQ
 */
function attr_imgjs($content)
{
    preg_match_all('/<img src=\"(.*?)\"/', $content, $img_src);

    foreach ($img_src[0] as $k => $v) {


        $imgUrlNew = $img_src[1][$k];

        //判断图片链接地址是否合法 不对则拼接
        if(substr($imgUrlNew,0,1) != 'h'){
            //判断环境
            if(ENVIRONMENTSTATUS == 'Reality'){
                $imgUrlNew = 'http://opensns.lz517.com'.$imgUrlNew;
            }else{
                $imgUrlNew = 'http://opensns.lz517.cn'.$imgUrlNew;
            }
        }

        $content = str_replace($v, '<img onclick="clickImg('.$k.')" src="' .$imgUrlNew. '"', $content);
    }
...
/**
 * 给链接添加js内部跳转事件 自用 资讯详情
 * @param $content
 * @return mixed|string
 * autor:HQ
 */
function attr_linkjs($content)
{

    preg_match_all('/<a href=\"http:\/\/app?(.*?)\"/', $content, $link_src);
    ...
    if($link_src[0][0]){
        ...
        //循环替换数据
        $i = 1;
        foreach ($link_src[0] as $k => $v) {

            $aUrlNew = '\''."http://app".$link_src[1][$k].'\'';

            $urlVal = substr($link_src[1][$k],'6');
            array_push($urlValData,$urlVal);

            $content = str_replace($v, '<a style="color:#0000cc" id="jumpId'.$i.'" value='.$urlVal.' onclick="clickUrlForInsaid('.$aUrlNew.')" ', $content);
//            $content = str_replace($v, '<img onclick="clickImg('.$k.')" src="' .$imgUrlNew. '"', $content);
        $i++;
        }
    }

    return [$content,$i,$urlValData];
}
