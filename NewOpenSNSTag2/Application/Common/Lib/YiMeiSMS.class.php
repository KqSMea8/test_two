 ...
 * @package Common\Lib
 */
class YiMeiSMS implements SMSChannel
{
    //发送即时短信
    ...
    //发送批量短信
    private $batchSmsUrl = "http://sdk4report.eucp.b2m.cn:8080/inter/sendBatchOnlySMS";
    //获取上行短信
    ...
    //用户序列号
    private $uName = "6SDK-EMY-6688-RKYOL";
    //用户密码
    private $uPwd = "930432";//TODO 密码加密和解密，让账号更安全
    //消息ID
    ...
    private function execPostRequest($url, $fields)
    {
        ...
        $result = http($url,$fields_string);
//		$myCurl = new MyCurl($linkurl);
//		$myCurl -> createCurl();
//		$result = $myCurl->__tostring();
    ...
    private function analysisResult($rs)
    {
        ...
        if ($arr['error'] == 0)
        {
            ...
            //$arr['info'] = iconv("UTF-8", "gbk", $arr['info']);
            return $arr;
        }
     ...
     */
     public function addRegisterInfo($cdkey,$password,$ename,$linkman,$phonenum,$mobile,$email,$fax,$address,$postcode)
     {
         ...
         $result = $this -> execPostRequest($this -> registerRegistInfoUrl, $fields);
         $rs = $this -> analysisResult($result);
         return $rs;
     }
     ...
     * QQ:2322523770
     */
    public function sendbatchSms($phones,$content,$sendtime){

        $content = iconv("UTF-8", "gbk", $content);
        $fields = [
            'appId'=>urlencode($this -> uName),
            'mobiles'=>$phones,
            'content'=>urlencode($content),
            'timerTime'=>urldecode($sendtime),
//            'requestTime'=>urldecode(microtime()),
            'requestTime'=>urldecode(microtime()),
            'requestValidPeriod'=>urldecode(time()),
        ];

        $jsonMap = json_encode($fields);
        $result = $this -> execPostRequest($this -> batchSmsUrl, $jsonMap);
