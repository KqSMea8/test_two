...
class ModuleModel extends Model
{
    ...
    public function getAll($is_installed = '')
    {
        ...
        if ($module === false) {
            ...
            Log::write('最后一步取出模板数据为 | '.json_encode($module));
    ...
    public function checkCanVisit($name)
    {
        ...
        foreach ($modules as $m) {
            if (isset($m['is_setup']) && $m['is_setup'] == 0 && $m['name'] == ucfirst($name) && $m['name'] != 'Wechat') {
    ...
    public function getModule($name)
    {
        ...
        if ($module === false || $module == null) {
            $m = $this->getInfo($name);
            if(ucfirst($name) == 'Wechat'){
                Log::write('取出的微信模板数据为空'.json_encode($m));
            }
            if ($m != array()) {
            ...
            if(ucfirst($name) == 'Wechat'){
                Log::write('取出的微信模板数据为'.json_encode($module));
            }
