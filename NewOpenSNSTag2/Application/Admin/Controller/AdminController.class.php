...
class AdminController extends Controller
{
    ...
    protected function _initialize()
    {
        ...
        if ($access === false) {
            ...
            if ($dynamic === null) {
                ...
                //获取当前访问页面的URL 主要是规避如果权限中加参数的问题
                $visitUrl = substr($_SERVER["QUERY_STRING"] , 3);
                $visitUrlArr = explode('.',$visitUrl);
                if($visitUrlArr[1]){
                    $visitUrl = $visitUrlArr[0];
                }

                if (!$this->checkRule($rule, array('in', '1,2')) && !$this->checkRule($visitUrl, array('in', '1,2'))) {
    ...
    public function getModules()
    {
        ...
        //根据sort字段排序
        $sortArr = [];
        foreach($modules as $tmpK=>$tmpV){
            $sortArr[$tmpK] = empty($tmpV['sort'])?0:$tmpV['sort'];
        }
...
//		$menus = $this->getSpecialMenu($menus);
	 ...
	 *
	 */
	private function getSpecialMenu($menus){

		$childMenu = $menus['child'];
		foreach($childMenu as $groupName=> &$list){
			//活动分类直接显示在子菜单中
			if($groupName  == '活动分类管理' ){
				...
				foreach($tree as $v){
					...
					$menus['child'][$groupName][] = [
						"id" 		=> $v['id'],
						"pid"		=> $v['id'],
						"title" 	=> $v['title'],
						"url" 		=> 'Admin/Event/event/id/'.$v['id'],
						"tip" 		=> '' ,
					];
    ...
    public function unsetSModuleData(){
        S('common_module_' . strtolower('Wechat'), null);
        S('common_module_' . strtolower('wechat'), null);
    }
}
