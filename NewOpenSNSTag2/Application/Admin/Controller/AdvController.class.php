...
class AdvController extends AdminController
{
    ...
    public function editPos()
    {
        ...
        if (IS_POST) {
            ...
            if ($pos['type'] == 2) {
                ...
                $builder->keyRadio('style', '轮播_风格', '', array(1 => 'TouchSlider 风格', 2 => 'KinmaxShow 风格'))->keyDefault('style', 1);
    ...
    public function editAdv()
    {
        ...
        if (IS_POST) {
            ...
            //验证链接地址填写是否合法
            if(I('url')){
                $error .= $this->checkUrlYesOrNo(I('url'));
            }
    ...
    //检查链接地址跳转详情页是否合法
    private function checkUrlYesOrNo($url){

        //http://opensns.lz517.com/?from=gongxiao&id=111

        $return = '';
        //获取到跳转的位置 供销详情页
        $gxptUrl = explode('&id=',explode('from=',$url)[1]);

        if($gxptUrl[0] == 'gongxiao' && $gxptUrl[1]){
            $GxptData = (new ProductModel())->where(['status'=>1,'check_status'=>2,'id'=>$gxptUrl[1]])->find();
            if(!$GxptData){
                $return.= "请填写已启用且审核通过的产品ID！";
            }
        }
