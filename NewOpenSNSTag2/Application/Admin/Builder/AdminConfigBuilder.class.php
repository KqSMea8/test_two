...
class AdminConfigBuilder extends AdminBuilder
{
    ...
    //后台咨询专用
    //如果$buyZDserver有效的话走只读
    public function keySort($name, $title, $subtitle = null,$buyZDserver)
    {
        if($buyZDserver){
            return $this->key($name, $title, $subtitle, 'readonly');
        }else{
     ...
     * @auth 黄庆
     */
    public function keyButton($name, $title, $subtitle = null,$opt)
    {
        return $this->key($name, $title, $subtitle, 'oneButton',$opt);
    }
     ...
     * @auth 陈一枭
     */
    public function keyNormsPrice($name, $title, $subtitle = null)
    {
        return $this->key($name, $title, $subtitle, 'normsPrice');
    }
    ...
    //后台商品分类专属status
    public function keyCategoryStatus($name = 'status', $title = '状态', $subtitle = null)
    {
        $map = array(3 => L('_DISABLE_'), 1 => L('_ENABLE_'));
        return $this->keySelect($name, $title, $subtitle, $map);
    }

    //后台分类专属status
    public function keyStatusNew($name = 'status', $title = '状态', $subtitle = null,$map)
    {
        return $this->keySelect($name, $title, $subtitle, $map);
    }
    ...
    public function keyClassRadio($name, $title, $subtitle = null, $options)
    {
        return $this->key($name, $title, $subtitle, 'classRadio', $options);
    }
     ...
     * @param null $subtitle
     */
    public function keyAppUpdateFile($name, $title, $subtitle = null){
        return  $this->key($name,$title,$subtitle,'appUpdateFile');
    }
    ...
    //二手商品使用
    public function keySingleImageGoods($name, $title, $subtitle = null)
    {
        return $this->key($name, $title, $subtitle, 'singleImageGoods');
    }
    ...
    public function keyOthers($name,$title,$subtitle = null,$type,$opt = null){

        return $this->key($name,$title,$subtitle,$type,$opt);
    }
    ...
    public function GXPTdisplay()
    {
        ...
        parent::display('admin_GXPTconfig');
    }
    ...
    public function newsDisplay()
    {
        ...
        parent::display('adminNews_config');
    }

    /**
     * keyChosen  多选菜单  TODO HQ 补充 调用此方法时 如果展示页面 调用了group方法 则需要将chosen方法放在第一页中 这是个BUG！！
     * @param $name
     * @param $title
     * @param null $subtitle
     * @param $options
     ...
     * @param       $status 判断这个分组显不显示
     * @return $this
     * @auth 肖骏涛
     */
    public function group($name, $list = array(),$status = true)
    {
        if($status){
