...
class AdminListBuilder extends AdminBuilder
{
     ...
     * @author HQ
     */
    public function buttonKAModalPopup($url, $params, $title, $attr = array())
    {
        //$attr中可选参数，data-title：模态框标题，target-form：要传输的数据
        $attr['modal-url'] = $this->addUrlParam($url, $params);
        $attr['data-role'] = 'modal_popup_new';
        return $this->button($title, $attr);
    }
     ...
     * @author HQ
     */
    public function buttonJpushModalPopup($url, $params, $title, $attr = array())
    {
        //$attr中可选参数，data-title：模态框标题，target-form：要传输的数据
        $attr['modal-url'] = $this->addUrlParam($url, $params);
        $attr['data-role'] = 'modal_popup';
...
//            $this->setSearchPostUrl('');
    ...
    {
        $key = array('name' => $name, 'title' => $title, 'type' => $type, 'opt' => $opt, 'width' => $width,'tip'=>$title);
        $this->_keyList[] = $key;
        return $this;
    }
    ...
    public function keyGxptStatus($name = 'status', $title = '状态')
    {
        $map = [ -1 => '<div style="color: red">'.L('删除').'</div>',2 => '<div style="color: red">'.L('禁用').'</div>',1 => '<div style="color: #008000">'.L('_ENABLE_').'</div>'];
        return $this->key($name, $title, 'status', $map);
    }

    public function keyNewsStatus($name = 'status', $title = '状态')
    {
        $map = array(-1 => L('_DELETE_'), 0 => L('_DISABLE_'), 1 => L('_ENABLE_'), 2 => L('_UNAUDLITED_'), 3 => L('已过期'));
        return $this->key($name, $title, 'status', $map);
    }

    public function keyGoodsStatus($name = 'status', $title = '状态')
    {
        $map = array(1 => '<div style="color: #008000">'.L('已上架').'</div>', 2 => '<div style="color: red">'.L('已下架').'</div>', 3 => '<div style="color: red">'.L('已删除').'</div>', 4 => '<div style="color: #0000ff">'.L('已过期').'</div>');
        return $this->key($name, $title, 'status', $map);
    }
	...
	public function keyETimeText($name = 'eTime', $title = '是否结束')
	{

		$key = array('name' => $name, 'title' => $title, 'type' => 'eTime', 'opt' => '', 'width' => '100px');
		$this->_keyList[] = $key;
		return $this;
	}


	public function keyMobile($name = 'mobile', $title = '手机号码')
	{

		$key = array('name' => $name, 'title' => $title, 'type' => 'mobile', 'opt' => '', 'width' => '100px');
		$this->_keyList[] = $key;
		return $this;
	}

	public function keyUsername($name = 'username', $title = '用户名')
	{

		$key = array('name' => $name, 'title' => $title, 'type' => 'username', 'opt' => '', 'width' => '100px');
		$this->_keyList[] = $key;
		return $this;
	}

	public function keyCuid($name = 'cuid', $title = '顾客ID')
	{

		$key = array('name' => $name, 'title' => $title, 'type' => 'cuid', 'opt' => '', 'width' => '100px');
		$this->_keyList[] = $key;
		return $this;
	}
...
//            return "<a href='" . $value['space_url'] . "' target='_blank'>[{$value[uid]}]" . $value['nickname'] . '</a>'; TODO 修改列表用户ID
            return "<a href='" . $value['space_url'] . "' target='_blank'>" . $value['nickname'] . '</a>';
        });
        ...
        /* 加入第二个列表 HQ*/
        $this->assign('listTop',$this->_dataTop);
    ...
    public function doSetStatus($model, $ids, $status = 1)
    {
        ...
        foreach ($ids as $id) {
            if($status == -1){
                $map['url'] = "Admin/News/index/cate/{$id}";
                D('Menu')->where($map)->delete();
            }
        }
    ...
    private function createDefaultGetUrlFunction($pattern)
    {
        ...
        return function ($item) use ($pattern, $fun) {
            ...
            $pattern = str_replace('@@@', $item['goods_id'], $pattern);
			$pattern = str_replace('***', $item['cuid'], $pattern);
			$pattern = str_replace('$$$',$item['status'], $pattern);
			$pattern = str_replace('!!!',$item['event_id'], $pattern);
			$pattern = str_replace('%%%',$item['year_service_id'], $pattern);
			$pattern = str_replace('&&&',$item['inspection_id'], $pattern);
			$pattern = str_replace('^^^',$item['inspection_store_id'], $pattern);
			$pattern = str_replace('~~~',$item['uid'], $pattern);
