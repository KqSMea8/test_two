...
//        var_dump($this->getLastSql());die;
    ...
    public function getOrderMemberListByPage($map = [], $page = 1, $order = 'orde.update_time desc', $field = '*', $r = 20)
    {
        ...
        $totalCount = $this->alias("orde")
            ...
            ->join("LEFT JOIN jpgk_wechat_order_appraise oa on oa.order_id = orde.id")//评价表
            ->where($map)
            ->count();
...
//        var_dump($this->getLastSql());die;
    ...
    public function getDataByOrderId($map = [],$field = '*'){
        $list = $this->alias("orde")
            ...
            ->field($field)
            ->select();
        file_put_contents("huangqing.log","获取sql查询语句|||".$this->getLastSql()."\r\n",FILE_APPEND);
        return $list;
    }
    ...
    public function getOrderDataByStatus($map,$field,$type=0,$order="item.urgent_level asc,orde.create_time desc"){//0客服，1维修，2消杀，3清洗
...
//                $dv['order_code'] = substr($dv['order_code'],0,10)."...";
     ...
     * @return mixed
     */
    public function getEnterpriseOrderDataByStatus($map,$field,$type=0,$order="item.urgent_level asc,orde.create_time desc"){//0客服，1维修，2消杀，3清洗
        $data = $this->alias("orde")
            ->join("jpgk_wechat_order_item as item on orde.id = item.order_id")
            ->join("left join jpgk_wechat_member member on member.uid=orde.workers_id")
            ->join("left join jpgk_wechat_facilitator faci on faci.id=orde.facilitator_id")
...
//                $dv['order_code'] = substr($dv['order_code'],0,10)."...";
                ...
                switch($dv['order_type']){
                    case 1:
                        $dv['order_type_text'] = "门店维修";
                        $dv['url'] = "/index.php?s=/Enterprise/CustomerServices/orderDetail/order_id/".$dv['id']."/type/".$dv['order_type']."/is_enterprise/1".".html";
                        if($type == 1){
                            $dv['url'] = "/index.php?s=/Enterprise/storeMaintain/orderDetail/order_id/".$dv['id']."/is_enterprise/1".".html";
                        }
                        if($type == 4){
                            $dv['url'] = "/index.php?s=/Enterprise/DistributionSupervisor/orderDetail/order_id/".$dv['id']."/type/".$dv['order_type']."/fid/".$dv['facilitator_id']."/is_enterprise/1".".html";
                        }
                        if($type == 5){
                            $dv['url'] = "/index.php?s=/Enterprise/DistributionSupervisor/maintainOrderDetail/order_id/".$dv['id']."/is_enterprise/1".".html";
                        }
                        break;
                    case 2:
                        $dv['order_type_text'] = "门店消杀";
                        $dv['url'] = "/index.php?s=/Enterprise/CustomerServices/orderDetail/order_id/".$dv['id']."/type/".$dv['order_type']."/is_enterprise/1".".html";
                        if($dv['is_year'] == 1 && in_array($dv['order_state'],[11,3]) && $dv['is_main'] == 1){
                            $dv['url'] = "/index.php?s=/Enterprise/CustomerServices/showCleanKillYearOrder/order_id/".$dv['id']."/is_enterprise/1".".html";
                        }
                        if($type == 2){
                            $dv['url'] = "/index.php?s=/Enterprise/storeCleanKill/orderDetail/order_id/".$dv['id']."/is_enterprise/1".".html";
                        }
                        if($type == 4){
                            $dv['url'] = "/index.php?s=/Enterprise/DistributionSupervisor/orderDetail/order_id/".$dv['id']."/type/".$dv['order_type']."/fid/".$dv['facilitator_id']."/is_enterprise/1".".html";
                            if($dv['is_year'] == 1 && in_array($dv['order_state'],[11,3]) && $dv['is_main'] == 1){
                                $dv['url'] = "/index.php?s=/Enterprise/DistributionSupervisor/showCleanKillYearOrder/order_id/".$dv['id']."/is_enterprise/1".".html";
                            }
                        }
                        if($type == 5){
                            $dv['url'] = "/index.php?s=/Enterprise/DistributionSupervisor/cleanKillOrderDetail/order_id/".$dv['id']."/is_enterprise/1".".html";
                        }
                        break;
                    case 3:
                        $dv['order_type_text'] = "烟道清洗";
                        $dv['url'] = "/index.php?s=/Enterprise/CustomerServices/orderDetail/order_id/".$dv['id']."/type/".$dv['order_type']."/is_enterprise/1".".html";
                        if($type == 3){
                            $dv['url'] = "/index.php?s=/Enterprise/storeCleaning/orderDetail/order_id/".$dv['id']."/is_enterprise/1".".html";
                        }
                        if($type == 4){
                            $dv['url'] = "/index.php?s=/Enterprise/DistributionSupervisor/orderDetail/order_id/".$dv['id']."/type/".$dv['order_type']."/fid/".$dv['facilitator_id']."/is_enterprise/1".".html";
                        }
                        if($type == 5){
                            $dv['url'] = "/index.php?s=/Enterprise/DistributionSupervisor/cleaningOrderDetail/order_id/".$dv['id']."/is_enterprise/1".".html";
                        }
                        break;
                }
            }
    ...
    public function getYearBabyOrderList($map = [], $order = 'orde.update_time desc', $field = '*' ){
        $list = M('WechatYearServiceTime')->alias("yst")
            ...
            ->join("jpgk_wechat_order_item as item on yst.order_id = item.order_id")
            ->join("LEFT JOIN jpgk_wechat_facilitator as faci on orde.facilitator_id = faci.id")
            ...
            ->field($field)
            ->select();
//        echo M('WechatYearServiceTime')->getLastSql();
        return $list;
    }
 ...
 * @return array
 */
    public function getYearOrderListForExcel($map = [], $order = 'orde.create_time desc', $field = '*'){
     ...
     * 获取一条年订单数据
     */
    public function getYearOrderDataByMap($map = [],$field="*"){
        $data = $this->alias("orde")
            ->join("jpgk_wechat_order_item as item on orde.id = item.order_id")
            ->where($map)
            ->field($field)
